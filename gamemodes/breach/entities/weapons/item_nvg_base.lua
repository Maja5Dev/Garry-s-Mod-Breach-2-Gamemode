...
function SWEP:GlobalCLThink()
	if self.DLightLevel > 0.1 then
		self.Dlight = DynamicLight(LocalPlayer():EntIndex(), false)
		if self.Dlight then
...
function SWEP:NVGSettings()
	return {
		...
		*/
		...
		--{"Color", "slider", {self.NVG.colour, 0, 5, 0}, function(value) self.NVG.add_b = value self.NVG.add_r = value end},
		{"Color", "slider", {self.NVG.add_g, 0.2, 2, 0.2}, function(value) self.NVG.add_g = value end},
...
function SWEP:SecondaryAttack()
	if CLIENT then
		if self.NextSettingsChange < CurTime() then
			...
			local nvg_settings = self:NVGSettings()
