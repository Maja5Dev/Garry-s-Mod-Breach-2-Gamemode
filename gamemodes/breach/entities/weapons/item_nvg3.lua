...
local function DrawNVG3EntitiesInfo(ents)
	for k,ent in pairs(ent) do
		if ent:IsPlayer() or ent:IsNPC() then
			...
			if dist < 2048 then
				...
				if BLUE_NVG_ENTITY_NAMES[ent:GetClass()] then
					if BLUE_NVG_ENTITY_NAMES[ent:GetClass()] == true then
						continue
					else
						name = BLUE_NVG_ENTITY_NAMES[ent:GetClass()]
					end
				end
...
SWEP.DefaultNVG = {
	...
	effect = function(nvg, tab)
		...
		DrawNVG3EntitiesInfo(ents.GetAll())
	end,
...
function SWEP:DrawHUD()
	...
	if self.Enabled == false then
		draw.Text({
			text = "Primary attack puts on the NVG",
			pos = {ScrW() / 2, ScrH() - 6},
			font = "BR2_ItemFont",
			color = Color(255,255,255,80),
			xalign = TEXT_ALIGN_CENTER,
