...
function SWEP:DrawHUD()
	if self.Enabled and self.BatteryLevel > 0 then
		for k,ent in pairs(ents.GetAll()) do
			if ent:IsPlayer() or ent:IsNPC() then
				...
				if dist < 2048 then
					...
					if translate_names[ent:GetClass()] then
						if translate_names[ent:GetClass()] == true then
							continue
						else
							name = translate_names[ent:GetClass()]
						end
					end
