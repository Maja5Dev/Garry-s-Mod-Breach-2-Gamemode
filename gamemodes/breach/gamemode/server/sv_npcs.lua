...
function TrackNPCs()
	...
	for k,ent in pairs(all_npcs) do
		...
		if #available_positions > 0 then
            ent.nextNPCMove = CurTime() + (GetBR2conVar("br2_npc_teleport_cooldown") or 45)  -- Cooldown before next move
            nextNPCTeleport = CurTime() + (GetBR2conVar("br2_npc_teleport_cooldown_global") or 45)  -- Global cooldown to avoid mass teleports
