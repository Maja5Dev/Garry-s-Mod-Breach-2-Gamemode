...
local br2_ci_logo_1 = Material("breach2/chaosinslogo.png", "noclamp smooth")
local br2_ci_logo_2 = Material("breach2/chaosinslogo.png", "noclamp smooth")

function Open_CI_SpawnMenu()
    ...
    info_menu_1_frame.Paint = function(self, w, h)
		...
		draw.Text({
			text = "Support Spawning: Chaos Insurgency",
        ...
        local info_table = {
            "Chaos Insurgency cells are infiltrating the facility.",
            "We utilize experimental weaponry and technology from the Foundation.",
            "Your objective is to seize high-value items and SCPs.",
            "D-Class personnel are valuable to us.",
            "Eliminate high-ranking Foundation staff and disrupt containment efforts.",
            "Choose your specialized squad and prepare to execute the breach.",
        }
		...
		surface.SetMaterial(br2_ci_logo_1)
        ...
        surface.SetMaterial(br2_ci_logo_2)
		surface.DrawTexturedRect(w - info_menu_1_logo_size - im1s, info_menu_exit_size + im1s, info_menu_1_logo_size, info_menu_1_logo_size)
    end
    ...
    for i=1, num_of_teams do
        ...
        if !istable(BR2_CI_TEAMS) then
            print("BR2_CI_TEAMS is not a table!", BR2_CI_TEAMS)
        end
        ...
        team_panel.Paint = function(self, w, h)
            ...
            for num,ply in pairs(BR2_CI_TEAMS[i]) do
                local pos_x = gap8
        ...
        team_panel_join.Think = function(self)
            ...
            for i2,v in ipairs(BR2_CI_TEAMS) do
        ...
        team_panel_join.DoClick = function(self)
            ...
            if isnumber(team_panel_join.team_num_found) and team_panel_join.team_num_found == i then
                net.Start("br_ci_teams_leave")
                net.SendToServer()

            elseif #BR2_CI_TEAMS[i] < 4 then
                net.Start("br_ci_teams_join")
                    net.WriteInt(i, 8)
                net.SendToServer()
            end
        end
...
function BR_OpenCIMenu()
    local size_w = 180
    local size_h = 190

    br_our_ci_frame = vgui.Create("DFrame")
    br_our_ci_frame:SetTitle("")
    br_our_ci_frame:ShowCloseButton(false)
    br_our_ci_frame:SetDraggable(false)
    br_our_ci_frame:SetPos(ScrW() - size_w, ScrH() - size_h)
    br_our_ci_frame:SetSize(size_w, size_h)
    br_our_ci_frame.nextUpdate = 0
    br_our_ci_frame.Think = function(self)
        if !IsValid(info_menu_1_frame) and br_our_ci_frame.nextUpdate < CurTime() then
            net.Start("br_ci_teams_update")
            net.SendToServer()
            br_our_ci_frame.nextUpdate = CurTime() + 1
        end
    end

    br_our_ci_frame.Paint = function(self, w, h)
